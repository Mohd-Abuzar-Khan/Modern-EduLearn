...
class EnrollmentServiceImplTest {
    ...
    void setUp() {
        ...
        ReflectionTestUtils.setField(enrollmentService, "courseServiceUrl", "http://course-service");
    ...
    void testEnrollSuccess() {
        ...
        when(restTemplate.getForEntity("http://course-service/courses/5", Map.class))
                .thenReturn(ResponseEntity.ok(Map.of("data", Map.of("isPublished", true))));
    ...
    void testEnrollUnpublishedCourse() {
        ...
        when(enrollmentRepository.existsByStudentIdAndCourseId(10L, 5)).thenReturn(false);
        when(restTemplate.getForEntity("http://course-service/courses/5", Map.class))
                .thenReturn(new ResponseEntity<>(Map.of("data", Map.of("isPublished", false)), HttpStatus.OK));

        // Act & Assert
        RuntimeException exception = assertThrows(RuntimeException.class, () -> enrollmentService.enroll(10L, 5));
