...
public class JobMatchServiceImpl implements JobMatchService {
    ...
    public JobMatch analyzeJobFit(Integer userId, Integer resumeId, Long matchId, String authToken) {
        ...
        try {
            ...
            job.setStrengths(result.has("strengths") ? result.get("strengths").asText() : "No specific strengths identified.");
            job.setWeaknesses(result.has("weaknesses") ? result.get("weaknesses").asText() : "No critical gaps identified.");
            job.setRecommendations(result.has("recommendations") ? result.get("recommendations").asText() : "No recommendations at this time.");
            ...
            job.setMatchScore(0);
            job.setRecommendations("The AI analysis encountered an error. Please ensure your resume is fully populated and try again.");
        ...
        try {
            ...
            if (sections instanceof List) {
                for (Object s : (List) sections) {
                    if (s instanceof Map) {
                        Map section = (Map) s;
                        sb.append(section.getOrDefault("sectionType", "")).append(":\n");
                        sb.append(section.getOrDefault("content", "")).append("\n\n");
                    }
                }
            }
            return sb.toString().isBlank() ? "No resume content found." : sb.toString();
        } catch (Exception e) {
